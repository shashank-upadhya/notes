    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <main className="p-4 md:p-6">
        ...
        <div className="space-y-3">
          {notes.length > 0 ? (
            notes.map((note: Note) => (
              <div key={note._id} className="bg-white p-4 rounded-xl shadow-md flex justify-between items-start gap-4">
                {/*ADDED THIS DIV TO WRAP TITLE AND CONTENT*/}
                <div className="flex-grow">
                  <h3 className="font-bold text-lg mb-1">{note.title}</h3>
                  {/*ADDED THIS LINE TO DISPLAY THE CONTENT*/}
                  <p className="text-gray-600 whitespace-pre-wrap">{note.content}</p>
                </div>
                <button onClick={() => onDelete(note._id)} className="text-gray-400 hover:text-red-500 flex-shrink-0">
                  <FaTrashAlt />
                </button>
              </div>
            ))
